...
export async function GET(request: NextRequest) {
  try {
    ...
    const items: (Item | null)[] = await Promise.all(
      itemsList.map(
        async (id) => await kv.get<Item | null>(KV_KEYS.item(id as string))
      )
    )
    ...
    const currentStocks = await Promise.all(
      validItems.map(async (item) => {
        ...
        if (latestRecord) {
          if (
            latestRecord.endQty !== null &&
            latestRecord.endQty !== undefined
          ) {
            currentStock = latestRecord.endQty
          } else {
            // 如果还未盘点，使用期初+采购量
            currentStock =
              (latestRecord.startQty || 0) + (latestRecord.receivedQty || 0)
          }
        }
    ...
    return NextResponse.json({ error: '获取当前库存失败' }, { status: 500 })
  }
}
